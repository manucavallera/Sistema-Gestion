...
interface Proveedor {
  id: number;
  razonSocial: string; // Cambié de nombre a razonSocial
  direccion: string;
  cuit: string;
  zona: string;
}
...
const ProveedoresPage: React.FC = (): JSX.Element => {
  ...
  const [nuevoProveedor, setNuevoProveedor] = useState<Omit<Proveedor, "id">>({
    razonSocial: "", // Cambié de nombre a razonSocial
    direccion: "",
    cuit: "",
    zona: "",
  });
  ...
  const guardarProveedor = async () => {
    if (
      nuevoProveedor.razonSocial && // Cambié de nombre a razonSocial
      ...
      try {
        ...
        setNuevoProveedor({ razonSocial: "", direccion: "", cuit: "", zona: "" }); // Cambié de nombre a razonSocial
  ...
  const iniciarEdicion = (proveedor: Proveedor) => {
    setNuevoProveedor({
      razonSocial: proveedor.razonSocial, // Cambié de nombre a razonSocial
  ...
  return (
    <div className='p-4 bg-background'>
      ...
      <div className='mb-4'>
        <input
          type='text'
          placeholder='Razón Social' // Cambié de Nombre a Razón Social
          value={nuevoProveedor.razonSocial} // Cambié de nombre a razonSocial
          onChange={(e) =>
            setNuevoProveedor({ ...nuevoProveedor, razonSocial: e.target.value }) // Cambié de nombre a razonSocial
          }
      ...
      <table className='min-w-full border border-gray-200'>
        <thead>
          <tr className='bg-gray-100'>
            <th className='px-4 py-2'>Razón Social</th> // Cambié de Nombre a Razón Social
        ...
        <tbody>
          {proveedores.map((proveedor) => (
            <tr key={proveedor.id} className='hover:bg-gray-50'>
              <td className='px-4 py-2'>{proveedor.razonSocial}</td> // Cambié de Nombre a Razón Social
