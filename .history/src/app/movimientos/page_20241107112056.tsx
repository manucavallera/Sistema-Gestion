...
import {
  getAllMovements,
  createMovement,
  getMovementsByClientId,
  getMovementsByProviderId,
  getMovementById,
...
const MovementsPage: React.FC = () => {
  ...
  const [nuevoMovimiento, setNuevoMovimiento] = useState<Movement>({
    monto: 0,
    tipo: "venta",
    fecha: "",
  });
  ...
  // Manejar eliminación de movimiento (lógica similar para editar)
  const handleEliminarMovimiento = async (id: number) => {
    setLoading(true);
    try {
      // Aquí tendrías que llamar a la función de eliminar del servicio, si la tuvieras
