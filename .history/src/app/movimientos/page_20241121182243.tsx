...
const MovimientosPage = () => {
  ...
  // Para editar
  const [movimientoAEditar, setMovimientoAEditar] = useState<Movimiento | null>(
    null
  );
  ...
  useEffect(() => {
    fetchMovimientos();
    fetchClientesYProveedores(); // Cargar clientes y proveedores al inicio
  }, []);
  ...
  const handleCreateMovimiento = async (e: React.FormEvent) => {
    ...
    const response = await fetch("/api/movimientos", {
      ...
      body: JSON.stringify({
        tipo,
        monto: Number(monto), // Asegúrate de convertir a número
  ...
  const cambiarEstadoMovimiento = async (id: number, nuevoEstado: string) => {
    const response = await fetch(`/api/movimientos/${id}`, {
      method: "PATCH", // Cambiar a PATCH si estás actualizando parcialmente
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const eliminarMovimiento = async (id: number) => {
    ...
    if (response.ok) fetchMovimientos(); // Actualizar la lista de movimientos
    } else {
      const errorData = await response.json();
    ...
    if (movimientoAEditar) {
      const response = await fetch(`/api/movimientos/${movimientoAEditar.id}`, {
        ...
        body: JSON.stringify({
          ...
          monto: Number(movimientoAEditar.monto), // Asegúrate de convertir a número
          estado: movimientoAEditar.estado,
          tipoPago: movimientoAEditar.tipoPago,
          clienteId: movimientoAEditar.clienteId,
          proveedorId: movimientoAEditar.proveedorId,
        }),
      });

      if (response.ok) {
        setMovimientoAEditar(null); // Resetear el movimiento a editar
        fetchMovimientos(); // Actualizar la lista de movimientos
      } else {
        const errorData = await response.json();
  ...
  return (
    <div className='max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md'>
      ...
      <form onSubmit={handleCreateMovimiento} className='space-y-4'>
        ...
        <div>
          ...
          <input
            type='number'
            value={monto}
            onChange={(e) => setMonto(Number(e.target.value))} // Asegúrate de convertir a número
            className='border rounded p-2 w-full'
            required
          />
        </div>
      ...
      {movimientoAEditar && (
        <form onSubmit={handleEditMovimiento} className='space-y-4 mt-6'>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setMovimientoAEditar({
                  ...movimientoAEditar,
                  monto: Number(e.target.value), // Asegúrate de convertir a número
                })
              }
              className='border rounded p-2 w-full'
              required
