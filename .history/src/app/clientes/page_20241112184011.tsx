...
const ClientesPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = clienteActual?.id ? `/api/clientes/${clienteActual.id}` : "/api/clientes";
      ...
      if (method === "POST") {
        ...
        setClientes(clientes.map((cliente) => (cliente.id === clienteGuardado.id ? clienteGuardado : cliente)));
      }
    ...
    <div className='p-6'>
      ...
      <form onSubmit={handleSubmit} className='mb-4'>
        ...
        <input
          ...
          onChange={(e) =>
            setClienteActual({ ...clienteActual, saldo: Number(e.target.value) })
          }
          required
          className='border p-2 mb-2 w-full'
        />
      ...
      <ul>
        {clientes.map((cliente) => (
          <li key={cliente.id} className='flex justify-between items-center mb-2'>
            <div>
              <strong>{cliente.razonSocial}</strong> - {cliente.cuit} - {cliente.zona} - Teléfono: {cliente.telefono} - Saldo: ${cliente.saldo}
            </div>
            <div>
              <button onClick={() => editarCliente(cliente)} className='bg-yellow-500 text-white p-1 mr-2'>
                Editar
              </button>
              <button onClick={() => eliminarCliente(cliente.id)} className='bg-red-500 text-white p-1'>
                Eliminar
              </button>
            </div>
          </li>
