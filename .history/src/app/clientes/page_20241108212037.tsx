...
const ClientesPage: React.FC = (): JSX.Element => {
    ...
    if (clienteEditando) {
      ...
      try {
        ...
        alert("Cliente actualizado con éxito.");
               ...
               setError("Error al actualizar el cliente: " + (err instanceof Error ? err.message : "Error desconocido"));
      }
    } else {
      ...
      try {
        ...
        setError("Error al agregar el cliente: " + (err instanceof Error ? err.message : "Error desconocido"));
      }
    }
  };
  ...
  const confirmarEliminacion = async (id: number) => {
    ...
    if (confirmacion) {
      try {
        await deleteClient(id); // Llama a la función para eliminar el cliente
        setClientes(clientes.filter(cliente => cliente.id !== id)); // Actualiza el estado para eliminar el cliente de la lista
        alert("Cliente eliminado con éxito.");
      } catch (err) {
        setError("Error al eliminar el cliente: " + (err instanceof Error ? err.message : "Error desconocido"));
      }
    }
  };
          ...
          {Array.isArray(clientes) && clientes.length > 0 ? (
            clientes.map((cliente) => (
              <tr key={cliente.id} className='hover:bg-gray-50'>
                <td className='px-4 py-2'>{                <td className='px-4 py-2'>{cliente.razonSocial}</td>
