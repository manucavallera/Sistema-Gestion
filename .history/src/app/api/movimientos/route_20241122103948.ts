  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Datos inválidos: monto debe ser un número positivo, tipo debe ser 'INGRESO' o 'EGRESO', estado debe ser 'PENDIENTE', 'COMPLETADO' o 'CANCELADO', y tipoPago debe ser uno de los métodos de pago válidos.",
        },
        { status: 400 }
      );
    }
      ...
      if (clienteId) {
        ...
        if (tipo === "INGRESO") {
          await prisma.cliente.update({
            where: { id: clienteId },
            data: {
              haber: new Decimal(cliente.haber).plus(monto), // Aumentar el haber
              saldo: new Decimal(cliente.saldo).plus(monto), // Aumentar el saldo
            },
          });
        } else if (tipo === "EGRESO") {
          await prisma.cliente.update({
            where: { id: clienteId },
            data: {
              debe: new Decimal(cliente.debe).plus(monto), // Aumentar el debe
              saldo: new Decimal(cliente.saldo).minus(monto), // Disminuir el saldo
            },
          });
        }
      }
      ...
      if (proveedorId) {
        ...
        if (tipo === "INGRESO") {
          await prisma.proveedor.update({
            ...
            data: {
              haber: new Decimal(proveedor.haber).plus(monto), // Aumentar el haber
              saldo: new Decimal(proveedor.saldo).plus(monto), // Aument ar el saldo
            },
          });
        } else if (tipo === "EGRESO") {
          await prisma.proveedor.update({
            where: { id: proveedorId },
            data: {
              debe: new Decimal(proveedor.debe).plus(monto), // Aumentar el debe
              saldo: new Decimal(proveedor.saldo).minus(monto), // Disminuir el saldo
            },
          });
        }
      }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // Actualizar el movimiento solo con el nuevo estado
