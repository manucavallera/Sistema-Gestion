...
async function actualizarSaldo(tipo, monto, entidadId, esCliente) {
  const entidad = esCliente
    ? await prisma.cliente.findUnique({ where: { id: Number(entidadId) } })
    : await prisma.proveedor.findUnique({ where: { id: Number(entidadId) } });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (estado === "COMPLETADO") {
      if (clienteId) {
        await actualizarSaldo(tipo, monto, clienteId, true);
      }
      if (proveedorId) {
        await actualizarSaldo(tipo, monto, proveedorId, false);
      }
    }
