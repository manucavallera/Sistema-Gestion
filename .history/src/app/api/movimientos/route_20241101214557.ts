...
export async function POST(request: Request) {
  ...
  // Validación para asegurar que se especifica un cliente o proveedor
  ...
  // Obtener el saldo anterior, dependiendo si es cliente o proveedor
  const saldoAnterior = await prisma.movimientoCuenta.findFirst({
    where: clienteId ? { clienteId } : { proveedorId },
    orderBy: { fecha: "desc" },
  });
  ...
  try {
    // Crear el nuevo movimiento en la cuenta corriente
...
export async function GET(request: Request) {
  ...
  // Validación para evitar obtener movimientos si no se proporciona ningún ID
  if (!clienteId && !proveedorId) {
    return NextResponse.json(
      { error: "Debe especificar un cliente o proveedor para filtrar" },
      { status: 400 }
    );
  }
