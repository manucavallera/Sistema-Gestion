...
const MovementsPage: React.FC = () => {
  ...
  const handleAgregarMovimiento = async () => {
    if (nuevoMovimiento.monto <= 0 || nuevoMovimiento.clienteId <= 0) {
    ...
    try {
      ...
      setMovimientos((prevMovimientos) => [...prevMovimientos, movimientoCreado]);
  ...
  const handleEliminarMovimiento = async (id: number) => {
    ...
    try {
      ...
      setMovimientos((prevMovimientos) => prevMovimientos.filter((movimiento) => movimiento.id !== id));
