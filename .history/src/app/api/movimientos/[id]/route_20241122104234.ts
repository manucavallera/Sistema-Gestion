...
export async function PATCH(
  ...
  try {
    ...
    if (body.estado === "COMPLETADO") {
      ...
      if (tipo === "EGRESO") { // Cambiar "DEBITO" por "EGRESO"
        // Verifica que clienteId no sea null
        if (clienteId !== null) {
          // Si es un egreso, restar del saldo del cliente
          ...
          return NextResponse.json(
            {
              error:
                "El movimiento es un egreso, pero no tiene un cliente asociado.",
            },
            { status: 400 }
          );
        }
      } else if (tipo === "INGRESO") { // Cambiar "CREDITO" por "INGRESO"
        // Verifica que proveedorId no sea null
        if (proveedorId !== null) {
          // Si es un ingreso, sumar al saldo del proveedor
      ...
      return NextResponse.json(
        {
          error:
            "El movimiento es un ingreso, pero no tiene un proveedor asociado.",
        },
        { status: 400 }
      );
    }
