...
export async function GET_CLIENTES(request: NextRequest) {
  try {
    const clientes = await prisma.cliente.findMany({
      select: {
        id: true,
        nombre: true, // Asegúrate de que este campo existe en tu modelo
...
export async function PATCH(request: NextRequest) {
  try {
    const { id } = request.query; // Asegúrate de que estás obteniendo el ID correctamente
    const body = await request.json();

    // Validación de datos
    const { estado, tipoPago } = body;
    ...
    const movimientoActualizado = await prisma.movimientoCuenta.update({
      where: { id: Number(id) },
      data: {
        estado: estado || movimientoExistente.estado, // Mantener el estado anterior si no se proporciona uno nuevo
        tipoPago: tipoPago || movimientoExistente.tipoPago, // Mantener el tipo de pago anterior si no se proporciona uno nuevo
      },
    });
