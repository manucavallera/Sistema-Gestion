  ...
  try {
    const nuevoCliente = await prisma.cliente.create({
      data: {
        ...
        cuit: data.cuit,
        zona: data.zona, // Aquí aseguramos que siempre se le asigne un valor válido
        telefono: data.telefono || "", // Puedes asignar una cadena vacía si no hay valor
        email: data.email || "", // Puedes asignar una cadena vacía si no hay valor
        saldo: data.saldo || 0,
      },
    });
