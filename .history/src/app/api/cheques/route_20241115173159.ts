...
export async function POST(request: Request) {
  ...
  try {
    const cliente = data.clienteRazonSocial
      ? await prisma.cliente.findUnique({ where: { razonSocial: data.clienteRazonSocial } })
      : null;
    const proveedor = data.proveedorRazonSocial
      ? await prisma.proveedor.findUnique({ where: { razonSocial: data.proveedorRazonSocial } })
      : null;
...
export async function GET(request: Request) {
  ...
  try {
    const cheques = await prisma.cheque.findMany({
      where: {
        ...(clienteRazonSocial && { cliente: { razonSocial: clienteRazonSocial } }),
        ...(proveedorRazonSocial && { proveedor: { razonSocial: proveedorRazonSocial } }),
      },
      include: {
        cliente: true,
        proveedor: true,
