...
export async function POST(request: NextRequest) {
  try {
    ...
    // Actualizar el debe y el haber
    if (movimiento.cliente) {
      // Suponiendo que los campos 'debe' y 'haber' existen en el modelo del cliente
      await prisma.cliente.update({
        where: { id: movimiento.cliente.id },
        data: {
          debe: { increment: movimiento.monto }, // Incrementar el debe con el monto del movimiento
          haber: { increment: monto }, // Incrementar el haber con el monto del pago
        },
      });
    }
