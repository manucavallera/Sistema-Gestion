...
export async function POST(request: NextRequest) {
  try {
    ...
    const { monto, proveedorId, clienteId, movimientoId } = body; // Incluye 'movimientoId'
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Datos inválidos. Asegúrate de que monto, proveedorId, clienteId y movimientoId estén presentes.",
        },
        { status: 400 }
      );
    }
