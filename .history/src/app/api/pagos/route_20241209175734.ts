...
export async function POST(request: NextRequest) {
  try {
    ...
    if (clienteId) {
      ...
      const nuevoDebeCliente = cliente.debe - monto;
      const nuevoHaberCliente =
        cliente.haber < 0 ? -cliente.haber : cliente.haber; // Asegurarse de que el haber sea positivo
    ...
    if (proveedorId) {
      ...
      const nuevoDebeProveedor = proveedor.debe - monto;
      const nuevoHaberProveedor =
        proveedor.haber < 0 ? -proveedor.haber : proveedor.haber; // Asegurarse de que el haber sea positivo
