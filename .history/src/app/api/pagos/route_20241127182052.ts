...
export async function POST(request: NextRequest) {
  try {
    ...
    // Obtener el monto total que debe el cliente
    const totalDebe = Number(movimiento.monto); // Este debería ser el monto total que debe el cliente
    const totalPagado = Number(nuevoPago.monto); // Monto que acaba de pagar

    // Calcular el nuevo saldo
    const nuevoSaldo = totalDebe - totalPagado;

    // Verificar que clienteId no sea null o undefined
    if (!movimiento.clienteId) {
      console.error("El movimiento no tiene un cliente asociado.");
      return NextResponse.json(
    ...
    await prisma.cliente.update({
      ...
      data: {
        ...
        debe: totalDebe, // Mantener el debe como el monto total
        haber: totalPagado, // Actualizar el haber con el monto pagado
      },
    });
