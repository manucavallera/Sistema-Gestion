...
export async function GET() {
  try {
    ...
    const pagosConDetalles = pagos.map((pago) => ({
      ...
      proveedor: pago.movimiento?.proveedor?.razonSocial ?? null, // Razón social del proveedor
      cliente: pago.movimiento?.cliente?.razonSocial ?? null, // Razón social del cliente
    }));
