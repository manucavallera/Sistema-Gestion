...
export async function POST(request: NextRequest) {
  try {
    ...
    // Actualizar el haber y el saldo del cliente
    const nuevoHaber = cliente.haber + monto; // Sumar el monto al haber
    const nuevoSaldo = nuevoHaber - cliente.de be; // Calcular nuevo saldo
...
export async function crearMovimiento(request: NextRequest) {
  try {
    ...
    if (typeof monto !== "number" || monto <= 0 || !clienteId) {
      return NextResponse.json(
        {
          error: "Datos inválidos. Asegúrate de que monto sea un número positivo y que clienteId esté presente.",
        },
        { status: 400 }
      );
    }
    ...
    const nuevoDebe = cliente.debe + monto; // Sumar el monto al debe

    // Actualizar la cuenta del cliente
    await prisma.cliente.update({
      where: { id: clienteId },
      data: {
        debe: nuevoDebe, // Actualizar el debe
      },
    });
