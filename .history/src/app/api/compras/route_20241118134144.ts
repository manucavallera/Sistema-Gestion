...
export async function POST(request: Request) {
  ...
  try {
    ...
    const nuevaCompra = await prisma.compra.create({
      data: {
        fecha: fecha,
        total: new Decimal(data.total), // Convertir a Decimal
...
async function actualizarSaldoProveedor(
  ...
  try {
    ...
    let nuevoSaldo = proveedor.saldo;

    // Convertir total a Decimal antes de realizar la operación
    const totalDecimal = new Decimal(total);

    if (estado === "COMPLETADO") {
      nuevoSaldo = nuevoSaldo.add(totalDecimal); // Aumentar saldo si el pago está completado
    } else if (estado === "CANCELADO") {
      nuevoSaldo = nuevoSaldo.sub(totalDecimal); // Disminuir saldo si el pago está cancelado
    }
