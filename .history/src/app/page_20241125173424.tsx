...
const links = [
  ...
  {
    href: "/movimientos", // Nuevo enlace para movimientos
    label: "Gestión de Movimientos",
    bgColor: "bg-yellow-500",
    hoverColor: "hover:bg-yellow-600",
  },
  {
    href: "/pagos", // Nuevo enlace para pagos
      ...
      <div className='mt-8 flex flex-col space-y-4'>
        {links.map((link) => (
          <Link key={link.href} href={link.href}>
            <button
              className={`px-4 py-2 text-white ${link.bgColor} rounded ${link.hoverColor} transition duration-300 ease-in-out`}
            >
              {link.label}
            </button>
          </Link>
