...
const ComprasPage = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelect Element>) => {
      ...
      setProveedores((prev) =>
        prev.map((prov) => {
          if (prov.id === compraCreada.proveedorId) {
            ...
            if (compraCreada.estado === "COMPLETADO") {
              nuevoSaldo -= compraCreada.total; // Resta para COMPLETADO
            } else if (compraCreada.estado === "CANCELADO") {
              nuevoSaldo -= compraCreada.total; // Resta para CANCELADO
