...
const ComprasPage = () => {
  ...
  const [clientes, setClientes] = useState<Cliente[]>([]); // Nuevo estado para los clientes
  ...
  const [razonSocialCliente, setRazonSocialCliente] = useState<string | null>(null); // Nuevo estado para la razón social del cliente
  ...
  const fetchClientes = useCallback(async () => { // Nueva función para obtener clientes
    setLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    fetchProveedores();
    fetchCheques();
    fetchClientes(); // Llamar a la función para obtener clientes
 ...
 e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>
  ...
  const handleChequeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (chequeSeleccionado) {
      ...
      const clienteSeleccionado = clientes.find(
        (cliente) => cliente.id === chequeSeleccionado.clienteId // Obtener el cliente asociado al cheque
      );
  ...
  const handleEdit = (compra: Compra) => {
    ...
    if (compra.chequeId) {
      ...
      const clienteSeleccionado = clientes.find(
        (cliente) => cliente.id === detalleCheque?.clienteId // Obtener el cliente asociado al cheque
      );
      setRazonSocialCliente(
        clienteSeleccionado ? clienteSeleccionado.razonSocial : null
      );
