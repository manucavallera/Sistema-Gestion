...
const ComprasPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const compraData = {
      fecha: new Date(`${nuevaCompra.fecha}T03:00:00`).toISOString(),
      total: nuevaCompra.total,
      proveedorId: nuevaCompra.proveedorId,
      metodoPago: nuevaCompra.metodoPago, // Incluir método de pago
    };
        ...
        <tbody>
          {compras.map((compra) => (
            <tr key={compra.id}>
              ...
              <td className='border border-gray-300 p-2'>
                {compra.proveedor
                  ? `${compra.proveedor.razonSocial} - Saldo: ${compra.proveedor.saldo}` // Mostrar saldo
