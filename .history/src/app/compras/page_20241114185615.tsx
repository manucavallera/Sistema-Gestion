...
const ComprasPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchCheques = async () => {
      setLoading(true);
      try {
        const response = await fetch("/api/cheques"); // Asegúrate de que esta API esté disponible
  ...
  const handleChange = (
    ...
    if (name === "proveedorId") {
      ...
      setNuevaCompra((prev) => ({ ...prev, [name]: Number(value) })); // Convertir total a número
    } else if (name === "metodoPago" && value === "CHEQUE") {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = is Editing ? `/api/compras/${nuevaCompra.id}` : "/api/compras";
...
/>
          ...
          {nuevaCompra.metodoPago === "CHEQUE" && (
            ...
            >
              ...
              {cheques
                .filter((cheque) => cheque.proveedorId === nuevaCompra.proveedorId)
                .map((cheque) => (
