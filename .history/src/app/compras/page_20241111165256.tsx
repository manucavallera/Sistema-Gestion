...
interface Compra {
  id: number;
  proveedorId: number;
  proveedorNombre: string; // Agregar el nombre del proveedor
  total: number;
  fecha: string;
}
...
const ComprasPage: React.FC = (): JSX.Element => {
  ...
  const [nuevaCompra, setNuevaCompra] = useState<
    Omit<Compra, "id" | "proveedorNombre">
  >({
    proveedorId: 0,
    total: 0,
    fecha: "",
  ...
  useEffect(() => {
    const cargarComprasYProveedores = async () => {
      ...
      try {
        ...
        // Asegúrate de que cada compra tenga el nombre del proveedor
        const comprasConProveedores = comprasResponse.map((compra: Compra) => {
          const proveedor = proveedoresResponse.data.find(
            (prov: Proveedor) => prov.id === compra.proveedorId // Especificar el tipo aquí
          );
          return {
            ...compra,
            proveedorNombre: proveedor ? proveedor.razonSocial : "Desconocido",
          };
        });
  ...
  const guardarCompra = async () => {
    console.log("Datos a guardar:", nuevaCompra); // Log para depuración
    ...
    if (
      ...
      try {
        if (compraEditando) {
          ...
          const updatedCompra = response; // No necesitas acceder a response.data si ya devuelve el objeto esperado
          setCompras(
            compras.map((compra) =>
          ...
          const compraCreada = response; // De nuevo, si ya devuelve el objeto esperado
        ...
        console.error("Error al guardar la compra:", err); // Log para depuración
    ...
    <div className='max-w-2xl mx-auto p-6 border-border -gray-300 rounded-lg shadow-lg bg-white'>
      ...
      <ul className='space-y-2'>
        {compras.map((compra) => (
          ...
          >
            {`Proveedor: ${compra.proveedorNombre}, Total: ${
              compra.total
            }, Fecha: ${format(new Date(compra.fecha), "dd/MM/yyyy HH:mm")}`}
            <div>
              <button
