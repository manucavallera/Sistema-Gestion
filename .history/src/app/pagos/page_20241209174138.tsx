...
interface Pago {
  id: number;
  monto: number; // Este es el monto que pagaste
  movimientoId: number;
  metodoPago: string;
...
interface Cheque {
  ...
  monto: number;
  utilizado: boolean;
  cliente: Cliente; // Ahora es un objeto de tipo Cliente
  proveedor: Proveedor; // Ahora es un objeto de tipo Proveedor
  banco: string;
  sucursal: string;
}
...
const PagosPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [pagosResponse, movimientosResponse, chequesResponse] =
          await Promise.all([
            fetch("/api/pagos"),
        ...
        if (!pagosResponse.ok) throw new Error("Error al obtener los pagos.");
        if (!movimientosResponse.ok)
          throw new Error("Error al obtener los movimientos.");
        if (!chequesResponse.ok)
          throw new Error("Error al obtener los cheques.");

        const [pagosData, movimientosData, chequesData] = await Promise.all([
          pagosResponse.json(),
          movimientosResponse.json(),
          chequesResponse.json(),
        ]);
        ...
        calcularDebeYHaber(movimientosData, pagosData);
      } catch (error) {
        handleFetchError(error, "datos");
      }
    };
  ...
  const calcularDebeYHaber = (movimientos: Movimiento[], pagos: Pago[]) => {
    const totalDebe = movimientos
      .filter((movimiento) => movimiento.tipo === "compra")
      .reduce((acc, movimiento) => acc + movimiento.monto, 0);

    const totalHaber = pagos.reduce((acc, pago) => acc + pago.monto, 0);

    setDebe(totalDebe);
    setHaber(totalHaber);
    setSaldo(totalDebe - totalHaber);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      resetForm();
      calcularDebeYHaber(movimientos, [...pagos, nuevoPago]);
    } catch (error) {
      ...
      >
        ...
        {chequeSeleccionado && (
          <div className='mt-4 p-4 border border-gray-300 rounded'>
            ...
            <p>Cliente: {chequeSeleccionado.cliente.razonSocial}</p>
            <p>Proveedor: {chequeSeleccionado.proveedor.razonSocial}</p>
