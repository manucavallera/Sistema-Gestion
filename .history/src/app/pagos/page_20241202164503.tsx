...
const PagosPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(editPagoId ? `/api/pagos/${editPagoId}` : "/api/pagos", {
  ...
  const resetForm = () => {
    setMonto(0);
    setMovimientoId(null setMetodoPago("EFECTIVO");
    setChequeId(null);
        ...
        <ul className='list-disc pl-5'>
          {pagos.map((pago) => (
            <li key={pago.id} className='mb-2'>
              Monto: {pago.monto}, Movimiento ID: {pago.movimientoId}, Método de Pago: {pago.metodoPago}, Razón Social: {pago.razonSocial || "N/A"}, Monto Movimiento: {pago.montoMovimiento !== null ? pago.montoMovimiento : "N/A"}
              <button onClick={() => handleEdit(pago)} className='ml-2 text-blue-500'>Editar</button>
              <button onClick={() => handleDelete(pago.id)} className='ml-2 text-red-500'>Eliminar</button>
            </li>
          ))}
        </ul>
      ) : (
