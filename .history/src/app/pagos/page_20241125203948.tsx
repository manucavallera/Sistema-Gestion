...
const PagosPage = () => {
  ...
  const [editPagoId, setEditPagoId] = useState<number | null>(null); // Para manejar la edición
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Error al crear/actualizar el pago.");
      }

      const nuevoPago: Pago = await response.json();
      if (editPagoId) {
        setPagos((prev) => prev.map((pago) => (pago.id === nuevoPago.id ? nuevoPago : pago)));
      } else {
      ...
      setEditPagoId(null); // Resetear el estado de edición
    } catch (error) {
      if (error instanceof Error) {
        ...
        console.error("Error desconocido:", error);
        setError("Error desconocido al crear/actualizar el pago.");
      }
    }
  };
  ...
  const handleEdit = (pago: Pago) => {
    ...
    setEdit PagoId(pago.id); // Establecer el ID del pago a editar
  };
      ...
      <ul className='list-disc pl-5'>
        {pagos.map((pago) => (
          <li key={pago.id} className='mb-2'>
            Monto: {pago.monto}, Movimiento ID: {pago.movimientoId}, Método de Pago: {pago.metodoPago}, Proveedor: {pago.proveedor}
