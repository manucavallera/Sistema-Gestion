...
const PagosPage = () => {
  ...
  const [chequeSeleccionado, setChequeSeleccionado] = useState<Cheque | null>(null); // Estado para el cheque seleccionado
  const [error, setError] = useState<string | null>(null);
  const [successMessage, setSuccessMessage] = useState<string | null>(null);
  const [editPagoId, setEditPagoId] = useState<number | null>(null);

  // Fetch initial data for pagos, movimientos, and cheques
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (monto <= 0 || movimientoId === null || !metodoPago) {
      setError("El monto debe ser positivo, el movimientoId y el metodoPago son obligatorios.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Error al crear o actualizar el pago.");
      }
      ...
      setSuccessMessage(editPagoId ? "Pago actualizado con éxito." : "Pago creado con éxito.");
      resetForm();
    } catch (error) {
  ...
  const resetForm = () => {
    ...
    setChequeId(null);
    setChequeSeleccionado(null); // Resetear el cheque seleccionado
    setEditPagoId(null);
  };
  ...
  return (
    <div className='max-w-2xl mx-auto p-4'>
      ...
      <form onSubmit={handleSubmit} className='bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4'>
        ...
        <div className='mb-4'>
          <label className='block text-gray-700 text-sm font-bold mb-2' htmlFor='movimientoId'>
            Movimiento
          </label>
          <select
            id='movimientoId'
        ...
        <div className='mb-4'>
          <label className='block text-gray-700 text-sm font-bold mb-2' htmlFor='metodoPago'>
            Método de Pago
          </label>
          <select
            id='metodoPago'
            ...
            onChange={(e) => {
              ...
              if (e.target.value !== "CHEQUE") {
                setChequeId(null setChequeSeleccionado(null); // Resetear el cheque seleccionado
              }
            }}
            required
            className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline'
        ...
        {metodoPago === "CHEQUE" && (
          <div className='mb-4'>
            <label className='block text-gray-700 text-sm font-bold mb-2' htmlFor='chequeId'>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                manejarCambioCheque(Number(e.target.value)); // Llama a la función para actualizar el cheque seleccionado
              }}
              required
              className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline'
            >
      ...
      <ul className='list-disc pl-5'>
        {pagos.map((pago) => (
          <li key={pago.id} className='mb-2'>
            Monto: {pago.monto}, Movimiento ID: {pago.movimientoId}, Método de Pago: {pago.metodoPago}, Razón Social: {pago.razonSocial}, Monto Movimiento: {pago.montoMovimiento}
            <button
