...
interface Pago {
  ...
  monto: number;
  movimientoId: number;
  metodoPago: string; // Agregar metodoPago aquí
  createdAt: string;
  updatedAt: string;
}
...
const PagosPage = () => {
  ...
  const [metodoPago, setMetodoPago] = useState<string>("EFECTIVO"); // Estado para metodoPago
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/pagos", {
        ...
        body: JSON.stringify({ monto, movimientoId, metodoPago }), // Incluir metodoPago
      });
