...
const PagosPage = () => {
  ...
  const [cheques, setCheques] = useState<Cheque[]>([]); // Nuevo estado para cheques
  const [monto, setMonto] = useState<number>(0);
  const [movimientoId, setMovimientoId] = useState<number | null>(null);
  const [metodoPago, setMetodoPago] = useState<string>("EFECTIVO");
  const [chequeId, setChequeId] = useState<number | null>(null); // Estado para el cheque seleccionado
  ...
  useEffect(() => {
    ...
    const fetchCheques = async () => {
      // Nueva función para obtener cheques
      try {
    ...
    fetchPagos();
    fetchMovimientos();
    fetchCheques(); // Llamar a la nueva función
  }, []);
    ...
    try {
      const response = await fetch(
        editPagoId ? `/api/pagos/${editPagoId}` : "/api/pagos",
        {
          ...
          body: JSON.stringify({ monto, movimientoId, metodoPago, chequeId }), // Incluir chequeId
        }
      );
  ...
  const resetForm = () => {
    ...
    setChequeId(null); // Resetear el chequeId
    setEditPagoId(null);
  };
  ...
  const handleEdit = (pago: Pago) => {
    ...
    setChequeId(pago.chequeId || null); // Asignar chequeId si existe
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (e.target.value !== "CHEQUE") {
                setChequeId(null); // Resetear chequeId si no es cheque
              }
            }}
            required
            className='shadow appearance-none border rounded w-full py-2 px -3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline'
