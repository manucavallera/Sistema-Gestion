...
const AgregarCheque = () => {
  ...
  const [entidadTipo, setEntidadTipo] = useState<"cliente" | "proveedor">("cliente");
    ...
    try {
      ...
      if (editingCheque) {
        setCheques((prevCheques) =>
          prevCheques.map((cheque) => (cheque.id === nuevoCheque.id ? nuevoCheque : cheque))
        );
 ...
 ```javascript
  ...
  const handleEdit = (cheque: Cheque) => {
    ...
    setEntidadId(cheque.cliente ? cheque.cliente.id : cheque.proveedor?.id || null);
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => setEntidadId(e.target.value ? Number(e.target.value) : null)}
            required
            className='mt-1 block w-full border border-gray-300 rounded-md p-2 focus:ring focus:ring-blue-500'
          >
      ...
      <ul className='mt-4 space-y-2'>
        {cheques.map((cheque) => (
          <li key={cheque.id} className='border p-2 rounded-md'>
            <p><strong>Fecha:</strong> {formatDate(cheque.fecha)}</p>
            <p><strong>Monto:</strong> ${cheque.monto}</p>
            <p>
              <strong>Entidad:</strong> {cheque.cliente ? cheque.cliente.razonSocial : cheque.proveedor?.razonSocial}
            </p>
