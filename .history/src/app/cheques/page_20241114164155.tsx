...
const AgregarCheque = () => {
  ...
  const buscarCliente = async () => {
    try {
      const response = await fetch(`/api/clientes?razonSocial=${encodeURIComponent(clienteRazonSocial)}`);
      if (!response.ok) {
  ...
  const buscarProveedor = async () => {
    try {
      const response = await fetch(`/api/proveedores?razonSocial=${encodeURIComponent(proveedorRazonSocial)}`);
      if (!response.ok) {
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Razón Social Cliente:</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Razón Social Proveedor:</label>
          <input
            type="text"
            value={proveedorRazonSocial}
            onChange={(e) => set ProveedorRazonSocial(e.target.value)}
            required
            className="mt-1 block w-full border border-gray-300 rounded-md p-2 focus:ring focus:ring-blue-500"
          />
        </div>
