...
const AgregarCheque = () => {
  ...
  const [clientes, setClientes] = useState<Cliente[]>([]); // Aquí se aplica el tipo
  const [proveedores, setProveedores] = useState<Proveedor[]>([]); // Aquí se aplica el tipo
  ...
  useEffect(() => {
    ...
    const fetchClientes = async () => {
      ...
      const data: Cliente[] = await response.json(); // Tipar la respuesta
      setClientes(data);
    };

    const fetchProveedores = async () => {
      const response = await fetch("/api/proveedores");
      const data: Proveedor[] = await response.json(); // Tipar la respuesta
      setProveedores(data);
    };
