...
export default function VentaPage() {
  ...
  const [formData, setFormData] = useState<{ fecha: string; total: number; clienteId: number }>({
    fecha: '',
    total: 0,
    clienteId: 0,
  });
    ...
    <div className="container mx-auto p-4">
      ...
      <form onSubmit={handleUpdate} className="mb-4">
        ...
        <div className="mb-4">
          ...
          <input
            type="number"
            name="clienteId value={formData.clienteId}
            onChange={handleChange}
            className="border rounded p-2 w-full"
            required
          />
        </div>
        <button type="submit" className="bg-blue-500 text-white p-2 rounded">Actualizar Venta</button>
      </form>
      <button onClick={handleDelete} className="bg-red-500 text-white p-2 rounded">Eliminar Venta</button>
    </div>
  );
}