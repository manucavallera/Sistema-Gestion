...
const VentasPage: React.FC = (): JSX.Element => {
  ...
  const [nuevaVenta, setNuevaVenta] = useState<Omit<Venta, "id">>({ clienteId: 0, monto: 0, fecha: "" });
    ...
    const cargarVentasYClientes = async () => {
      ...
      try {
        ...
        if (axios.isAxiosError(err)) {
          setError(err.response?.data?.message || "Error al cargar las ventas o los clientes");
        } else {
  ...
  const guardarVenta = async () => {
    ...
    try {
      if (ventaEditando) {
        ...
        setVentas(ventas.map((venta) => (venta.id === updatedVenta.id ? updatedVenta : venta)));
      } else {
      ...
      <div className='mb-4'>
        ...
        >
          ...
          {Array.isArray(clientes) && clientes.length > 0 ? (
            ...
            <option value='0' disabled>No hay clientes disponibles</option>
          )}
        </select>
        <input
          type='number'
