...
export const createSale = async (venta: {
  ...
  try {
    ...
    throw new Error("Error al crear la venta"); // Lanzar un error en lugar de devolver undefined
  }
};
...
export const updateSale = async (
  ...
  try {
    ...
    throw new Error(`Error al actualizar la venta con ID ${id}`); // Lanzar un error en lugar de devolver undefined
  }
};
...
export const getAllSalesWithClients = async (): Promise<
  (Sale & { razonSocial: string })[]
> => {
  try {
    ...
    const salesWithClients = await Promise.all(
      sales.map(async (sale: Sale) => {
        ...
        return {
          ...sale,
          razonSocial: client ? client.razonSocial : "Cliente no encontrado", // Manejar el caso donde el cliente no se encuentra
        };
      })
    );
    ...
    throw new Error(
      `Error al obtener ventas con clientes: ${
